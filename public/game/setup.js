...
export let c = document.createElement("canvas"),
  ...
  resizeCallback = () => {
    ...
    if (nameInput) {
      ...
      nameInput.style.height =
        Math.round(60 * (c.height / frame.height)) + "px";
    ...
    if (chatInput) {
      ...
      chatInput.style.height =
        Math.round(30 * (c.height / frame.height)) + "px";
  ...
  clickCallback = e => {
    let canvasX = Math.round(
        (e.clientX - marginLeft) * (frame.width / c.width)
      ),
      ...
      for (let i = 0; i < 2; i++) {
        for (let j = 0; j < 4; j++) {
          if (
            ...
            if (i == 0 && j == 0) {
              hairStyle--;
              if (hairStyle < -1) hairStyle = hairStylesCount - 1;
              redraw();
            } else if (i == 1 && j == 0) {
              hairStyle++;
              if (hairStyle == hairStylesCount) hairStyle = -1;
              redraw();
            } else if (i == 0 && j == 1) {
              characterBase--;
              if (characterBase < 0) characterBase = characterBases.length - 1;
              redraw();
            } else if (i == 1 && j == 1) {
              characterBase++;
              if (characterBase == characterBases.length) characterBase = 0;
              redraw();
            } else if (i == 0 && j == 3) {
              rotation--;
